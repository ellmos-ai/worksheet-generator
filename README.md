...
`config.json` holds the defaults (`material_dirs`, `icf_source`, `renderers`, `sprache`). Local, unversioned overrides belong in `config.local.json` (gitignored) — template: `config.local.example.json`.
